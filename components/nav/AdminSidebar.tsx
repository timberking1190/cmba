  ...
  return (
    <aside className="w-56 flex-shrink-0 bg-[#040100]/95 border-r border-white/8 flex flex-col h-screen sticky top-0">
      <div className="p-5 border-b border-white/8">
        ...
        <div className="mt-2 font-mono text-[9px] tracking-[3px] uppercase text-gray-600">Admin Platform</div>
      </div>

      <nav className="flex-1 overflow-y-auto py-3">
        {NAV.map((item, i) => {
          if ('section' in item) {
            return <div key={i} className="px-5 py-2 mt-3 font-mono text-[9px] tracking-[2px] uppercase text-gray-700">{item.section}</div>
          }
            ...
            <Link key={item.href} href={item.href}
              className={`flex items-center gap-3 px-5 py-2.5 text-sm transition-all border-l-2
                ${isActive
                  ...
                  : 'text-gray-500 border-transparent hover:text-white hover:bg-white/4'}`}>
              <Icon size={15} />
              <span>{item.label}</span>
              {item.badge && (
                <span className="ml-auto font-mono text-[9px] bg-red-600 text-black px-1.5 py-0.5">{item.badge}</span>
              )}
            </Link>
          )
        })}
      ...
      <div className="p-4 border-t border-white/8">
        <Link href="/" className="flex items-center gap-2 text-xs text-gray-600 hover:text-gray-400 mb-3">← Public Site</Link>
