  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <div className="font-mono text-[9px] tracking-[3px] uppercase text-red-500 mb-1">Communications</div>
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="font-mono text-[9px] tracking-[2px] uppercase text-gray-600">{a.category}</span>
                  <span className="font-mono text-[9px] text-gray-700">·</span>
                  <span className="font-mono text-[9px] text-gray-600">{new Date(a.published_at).toLocaleDateString('en-CA',{month:'short',day:'numeric',year:'numeric'})}</span>
                </div>
                <h3 className="font-medium text-white mb-1">{a.title}</h3>
                <p className="text-sm text-gray-500 leading-relaxed">{a.body?.slice(0,150)}{a.body?.length>150?'...':''}</p>
              </div>
              <button onClick={() => togglePublish(a.id, a.is_published)}
                className={`ml-4 font-mono text-[9px] tracking-[1px] px-3 py-1.5 border transition-colors
                  ${a.is_published ? 'border-green-500/30 text-green-400 hover:bg-red-600/10 hover:text-red-400 hover:border-red-600/30' : 'border-white/10 text-gray-600 hover:text-green-400 hover:border-green-500/30'}`}>
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={e=>{if(e.target===e.currentTarget)setShowNew(false)}}>
          <div className="bg-[#0a0400] border border-white/15 w-full max-w-lg">
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="font-mono text-[9px] tracking-[2px] uppercase text-gray-500 block mb-2">Title</label>
              ...
              <div>
                <label className="font-mono text-[9px] tracking-[2px] uppercase text-gray-500 block mb-2">Message</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="font-mono text-[9px] tracking-[2px] uppercase text-gray-500 block mb-2">Category</label>
