    ...
    <div className="min-h-screen">
      ...
      <div className="relative z-10 pt-24 px-4 md:px-14 pb-32 md:pb-20">
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <span className="font-mono text-[10px] tracking-[3px] uppercase text-red-500">RAMP Pathway</span>
        </div>
        ...
        {tab === 'pathway' && (
          <div>
            ...
            <div className="border border-white/8 p-6 mb-8">
              <div className="font-mono text-[9px] tracking-[3px] uppercase text-gray-500 mb-5">RAMP Certification Pathway — Basketball Alberta</div>
                ...
                {['Level 1 Official','Level 2 Official','Level 3 Official'].map((l,i) => (
                  <div key={i} className="flex items-center">
                    <div className={`border px-5 py-4 ${i===0?'border-red-600/40 bg-red-600/8':'border-white/15 bg-white/3'}`}>
                      <div className="font-mono text-[8px] tracking-[1px] uppercase text-gray-600 mb-1">Level {i+1}</div>
                      <div className="font-display text-sm tracking-wide text-white">{l}</div>
                      <div className="text-xs text-gray-600 mt-1">{['U13 eligible','U15 eligible','Senior eligible'][i]}</div>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {loading ? (
                <div className="col-span-3 text-center py-10 font-mono text-xs text-gray-600 tracking-[2px]">LOADING...</div>
              ) : courses.map(c => (
                <div key={c.id} className={`border ${c.is_featured?'border-red-600/30':'border-white/8'} bg-white/2 flex flex-col`}>
                  {c.is_featured && (
                    <div className="bg-red-600/15 px-4 py-1.5 border-b border-red-600/20 flex items-center gap-1.5">
                      <Star size={10} className="text-red-400"/><span className="font-mono text-[9px] tracking-[2px] uppercase text-red-400">Required</span>
                    </div>
                  )}
                  <div className="p-5 flex-1 flex flex-col">
                    ...
                    {c.certification && <div className="font-mono text-[9px] text-gray-600 mb-3">🏅 {c.certification}</div>}
                    <div className="flex items-center justify-between text-xs mb-4">
                      {c.duration_hours && <span className="text-gray-600">⏱ {c.duration_hours}h</span>}
        ...
        {tab === 'signals' && (
          <div>
            ...
            {loading ? (
              <div className="text-center py-10 font-mono text-xs text-gray-600 tracking-[2px]">LOADING SIGNALS...</div>
                  ...
                  {sigs.map(s => (
                    <div key={s.id}>
                      <button onClick={()=>setExpanded(expanded===s.id?null:s.id)}
                        ...
                        <span className="text-xs text-gray-600 hidden sm:block">{s.description}</span>
                        <span className={`font-mono text-[9px] text-gray-600 ml-2 transition-transform ${expanded===s.id?'rotate-90':''}`}>›</span>
                      </button>
                      {expanded===s.id && (
                        <div className="px-5 pb-4 ml-12 space-y-2">
                          ...
                          {s.mechanic && (
                            <div className="border border-white/8 bg-white/2 px-4 py-2.5 inline-block">
                              <div className="font-mono text-[9px] tracking-[2px] uppercase text-gray-600 mb-1">Mechanic</div>
        ...
        {tab === 'poe' && (
          <div className="max-w-3xl space-y-4">
            ...
            {loading ? (
              <div className="text-center py-10 font-mono text-xs text-gray-600 tracking-[2px]">LOADING...</div>
            ) : poe.map(p => (
              <div key={p.id} className={`border p-5 ${PRIORITY_STYLE[p.priority]||'border-white/8'}`}>
                <div className="flex items-start justify-between gap-4 mb-3">
                  ...
                  <span className={`font-mono text-[9px] px-2 py-1 tracking-[1px] flex-shrink-0 ${PRIORITY_BADGE[p.priority]}`}>{p.priority}</span>
                </div>
